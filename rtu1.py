				...
				while True:
					...
					if isUp:
						...
						if "PROCESS" in data.decode():
							...
							sockprocess.sendto(
								process_data.encode(), (ip, port2))
							#data3, addr3 = sockprocess.recvfrom(1024)
							#print(process_data, data3.decode())
					else:
						count = 0
						print("Connection between RTU and SCADA is down, rerouting")
						# error = "ERROR CONN DOWN"
						# sockhealth.sendto(error.encode(), (ip2, port))
						# sockprocess.sendto(error.encode(), (ip, port2))
						while count != 20:
							data, addr = sock_plc.recvfrom(1024)
							if "HEALTH" in data.decode():
								print("health")
							...
							if "PROCESS" in data.decode():
								...
								process_data = data.decode()
								sockprocess.sendto(
								process_data.encode(), (ip2, error_port))
								data3, addr3 = sockprocess.recvfrom(1024)
								print(process_data, data3.decode())
							count+=1
						isUp = True
